...
export default function RadialOrbitalTimeline({ timelineData }: RadialOrbitalTimelineProps) {
  ...
  const toggleItem = (id: number) => {
    setExpandedItems((prev) => {
      ...
      Object.keys(newState).forEach((key) => {
        if (Number.parseInt(key, 10) !== id) newState[Number.parseInt(key, 10)] = false;
      });
      newState[id] = !prev[id];
  ...
  const getStatusStyles = (status: TimelineItem["status"]): string => {
    switch (status) {
      case "completed":
        return "text-white bg-emerald-900/40 border-emerald-400";
      case "in-progress":
        return "text-white bg-blue-900/40 border-blue-400";
      case "pending":
      default:
        return "text-white bg-zinc-800 border-zinc-400";
    }
  };
    ...
    >
      <div className="relative w-full max-w-5xl h-full flex items-center justify-center">
        ...
        >
          ...
          <div className="absolute w-[30rem] h-[30rem] rounded-full border border-white/10" />
